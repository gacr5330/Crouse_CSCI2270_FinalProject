...
Monster dialogue_newchar()
{
	...
	int result = Conversations(
		"Choose your class",
		{"Fighter", "Rogue", "Mage"}).startConversation();

	switch(result)
	{
		// Fighter class favours health and strength
		case 1:
			return Monster(name, 35, 20, 10, 5, 0, 10.0, 1, "Fighter");
			break;

		// Rogue class favours dexterity and hit rate
		case 2:
			return Monster(name, 30, 5, 10, 20, 0, 15.0, 1, "Fighter");
			break;

		// Mage class Relies on Mana which gives extremely high hit rate and static attack damage
        case 3:
            return Monster(name, 25, 0, 10, 10, 30, 15.0, 1, "Mage");

		// Default case that should never happen, but it's good to be safe
		default:
			return Monster(name, 30, 10, 10, 10, 0, 10.0, 1, "Adventurer");
		break;
	}
}
